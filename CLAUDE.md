...
> **REWORK DESIGNED, NOT YET BUILT (2026-07-24) — see `ENDGAME_REWORK.md` (tracker #0069).** Playtest found the endgame both unwinnable-in-practice for a maxed kit *and* fully-funded before it's even reached (economy bug). The locked replacement turns the Final Siege into a scripted **three-visit** climax: cruise to it → guaranteed loss (real reset, unlocks "Preparation" upgrades incl. a *purchasable* Miracle) → climb back, get further, lose again → the Miracle fires (Cathedral revives mid-siege, unlocks free overpowered capstones) → continuation wins. Visits 1-2 unwinnable by tuning; the earned win is certain. The description below is the CURRENT (pre-rework) behavior.
