...
**Boss waves:** the tier's named boss is a heavily scaled-up brute (×1.8 power, ×4 HP on top of the wave multiplier) with a signature trait (see the tier table), leading an honor guard from the tier's composition. Past the Dragon Empress: the Final Siege countdown, then the gauntlet — and endless scaling after victory (see *Victory condition*).
