# Idle Kingdom — Design Spec

**This file is the source of truth for the game's design.** Design-first, not code-first: the
current `game.js` is a prototype mid-transition; the code gets redesigned around this spec.
Don't treat what the code currently does as a design constraint or authority — read it to
estimate implementation cost or find gaps, not to justify design choices. Mechanics, formulas,
balance values, and design decisions here must match what is actually implemented; update this
file whenever they change. Orientation and standing rules: `CLAUDE.md`. Milestone history and
current scope: `MILESTONES.md`.
